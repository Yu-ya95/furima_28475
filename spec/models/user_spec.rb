...
RSpec.describe User, type: :model do
  describe '#create' do
    ...
    it 'passwordが6文字以上の半角英数混合であれば登録できること' do
    ...
    it 'last_nameが全角(漢字・ひらがな・カタカナ)以外であれば登録できないこと' do
      @user.last_name = 'abc'
      @user.valid?
    ...
    it 'first_nameが全角(漢字・ひらがな・カタカナ)以外であれば登録できないこと' do
      @user.first_name = '123'
      @user.valid?
      expect(@user.errors.full_messages).to include('First name Full-width characters')
    end

    it 'last_name_kanaが全角(カタカナ)であれば登録できること' do
      @user.last_name_kana = 'カタカナ'
      expect(@user).to be_valid
    end

    it 'last_name_kanaが全角(カタカナ)以外であれば登録できないこと' do
      @user.last_name_kana = '漢字'
      @user.valid?
      expect(@user.errors.full_messages).to include('Last name kana Full-width katakana characters')
    end

    it 'first_name_kanaが全角(カタカナ)であれば登録できること' do
      @user.first_name_kana = 'カタカナ'
      expect(@user).to be_valid
    end

    it 'first_name_kanaが全角(カタカナ)以外であれば登録できないこと' do
